...
/// \brief Print flat 2D text over openGl videowindow
...
public:
    ...
    videowindow(int width, int height, const char* title)
        : _width(width), _height(height), _canvas_left_top_x(0), _canvas_left_top_y(0), _canvas_width(width), _canvas_height(height)
    {
        ...
        if (!win)
            throw std::runtime_error("Could not open OpenGL videowindow, please check your graphic drivers or use the textual SDK tools");
        glfwMakeContextCurrent(win);

        glfwSetWindowUserPointer(win, this);
        glfwSetMouseButtonCallback(win, [](GLFWwindow* w, int button, int action, int mods)
            {
                auto s = (videowindow*)glfwGetWindowUserPointer(w);
    ...
    //another c'tor for adjusting specific frames in specific tiles, this videowindow is NOT resizeable
    videowindow(unsigned width, unsigned height, const char* title, unsigned tiles_in_row, unsigned tiles_in_col, float canvas_width = 0.8f,
...

    {
        ...
        {
            std::cout << "Invalid videowindow's size parameter entered, setting to default values" << std::endl;
            canvas_width = 0.8f;
            canvas_height = 0.6f;
        ...
        glfwInit();
        // we don't want to enable resizing the videowindow
...
private:
    ...
    rect calc_grid(rect r, size_t streams)
    {
        if (r.w <= 0 || r.h <= 0 || streams <= 0)
            throw std::runtime_error("invalid videowindow configuration request, failed to calculate videowindow grid");
        float ratio = r.w / r.h;
...
// Struct to get keys pressed on videowindow
struct window_key_listener {
    int last_key = GLFW_KEY_UNKNOWN;

    window_key_listener(videowindow& win) {
...
void register_glfw_callbacks(videowindow& app, glfw_state& app_state)
{
