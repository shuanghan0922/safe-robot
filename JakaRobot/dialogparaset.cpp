    ...
    ui(new Ui::Dialogparaset)
{
  ...
  //  QMessageBox::information(this,"提示","参数构造函数执行次数"+QString::number(aa));
   ...
   // ui->lineEditopt->setText(QString::number(sysparaset.dayop));
    ...
    ui->lineEditplcport2->setText(QString::number(sysparaset.wslocport));
    ui->lineEditplcip3->setText(sysparaset.wslocip);
    ui->lineEditplcport3->setText(QString::number(sysparaset.wsremoteport));
  // ui->lineEditsecsip->setText(sysparaset.addsecs);
 //   ui->lineEditsecsport->setText(QString::number(sysparaset.portsecs));
    ui->lineEditpcip->setText(sysparaset.wsremoteip);
   // QString qstep;
...
void Dialogparaset::on_lineEditpcip_returnPressed()//PC IP
{
    ...
    sysparaset.wsremoteip=ui->lineEditpcip->text();
    qpcplcip=sysparaset.wsremoteip;
...
void Dialogparaset::on_lineEditplcip3_returnPressed()
{
    ...
    sysparaset.wslocip=ui->lineEditplcip3->text();
    qplcip=sysparaset.wslocip;
...
void Dialogparaset::on_lineEditplcport2_returnPressed()
{
    ...
    sysparaset.wslocport=ui->lineEditplcport2->text().toInt();
    qplcport.sprintf("%d",sysparaset.wslocport);
...
void Dialogparaset::on_lineEditplcport3_returnPressed()
{
    ...
    sysparaset.wsremoteport=ui->lineEditplcport3->text().toInt();
    qplcport.sprintf("%d",sysparaset.wsremoteport);
