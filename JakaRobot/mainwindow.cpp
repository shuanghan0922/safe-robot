...

{
    ...
    connect(wsserver,&WSServer::NewClient_conncted,this,&MainWindow::add_client);
    connect(wsserver,&WSServer::Client_disconncted,this,&MainWindow::remove_client);
    connect(wsserver,&WSServer::NewTextMessage,this,&MainWindow::S_recive);
    connect(wsserver,&WSServer::NewBinaryMessage,this,&MainWindow::S_reciveb);
    connect(wsserver,&WSServer::occur_error,this,&MainWindow::S_error);

    connect(this,&MainWindow::S_start,wsserver,&WSServer::Server_start);
    connect(this,&MainWindow::S_stop,wsserver,&WSServer::Server_stop);
    connect(this,&MainWindow::S_send,wsserver,&WSServer::Server_sendmsg);
    connect(this,&MainWindow::S_sendb,wsserver,&WSServer::Server_sendBinarymsg);
    connect(this,&MainWindow::S_remove,wsserver,&WSServer::remove_client);
    ...
    while(query.next())//指向第一条记录
    {
     ...
     //  ui->radioButtonmanul->setChecked(!query.value(0).toBool());
      ...
      // QMessageBox::information(0,"提示",sysparaset.addplc);
       ...
       sysparaset.wsremoteip=query.value(18).toString();
   ...
   //    sysparaset.boxfreshold=query.value(43).toFloat();
       ...
       sysparaset.wslocport=query.value(53).toInt();
       sysparaset.wslocip=query.value(54).toString();//
       sysparaset.wsremoteport=query.value(55).toInt();
...
******************/
   ...
   // QMessageBox::information(this,"提示",equparaset[0].id);//qx);
    ...
    ui->stackedWidget->move(0,80);//
    ui->stackedWidget->resize(1780,910);
    ui->line->hide(); //test边框
    ui->line_2->hide();
    ui->line_3->hide();
    ui->line_4->hide();
...
***************/
    ...
    //**************
        ...
        res=pthread_create(&pthbutton,NULL,Threadbutton,NULL);//启动相机线程
        if(res!=0)
        {
...
void MainWindow::paintEvent(QPaintEvent *)//画状态栏图形
{
    ...
    image.load(fn); //"/home/zm/testfinspushbutton/user.jpg");
...
//**************
    ...
    image.load(fn);  //"/home/zm/testfinspushbutton/PLC.png");
    //QPainter painter(this);
    pix = QPixmap::fromImage( image.scaled(size(), Qt::KeepAspectRatio) );
    //second_statusLabel->resize(20,20);//**********************************************************8
    ...
    image.load(fn); //"/home/zm/testfinspushbutton/photo.jpg");
    //QPainter painter(this);
    pix = QPixmap::fromImage( image.scaled(size(), Qt::KeepAspectRatio) );
    //second_statusLabel->resize(20,20);//**********************************************************8
    ...
    image.load(fn); //"/home/zm/testfinspushbutton/time.jpg");
    //QPainter painter(this);
    pix = QPixmap::fromImage( image.scaled(size(), Qt::KeepAspectRatio) );
    //second_statusLabel->resize(20,20);//**********************************************************8
...
/* 服务端操作 begin */
void MainWindow::S_recive(QString ClientInfo,QString message)
{
       ...
       // QJsonDocument doucment = QJsonDocument::fromJson(byteArray, &jsonError);
        if (!doucment.isNull())//&& (jsonError.error == QJsonParseError::NoError))
        {
            ...
            {
                QJsonObject object = doucment.object();
                if (object.contains("op"))//op 操作
                {
                    QString opvalue = object.value("op").toString();

                    if (opvalue=="sendfile")//文件
                    {
                    ...
                    if (opvalue=="video")//视频开启
                    {
                       QJsonObject msgobj = object.value("args").toObject();
                       /* data键 */
                       int sel = msgobj.value("select").toInt();
                       if(sel==1)camera_running=true;
                       else camera_running=false;
                    }

                    if (opvalue=="switch") //开关
                    {
                       QJsonObject msgobj = object.value("args").toObject();
                       QString novalue = msgobj.value("no").toString();
                       int stvalue = msgobj.value("state").toInt();
                       int kkidx;
                       QString qsl,qst;
                       gno=novalue;
                       gstate=stvalue;
                       kkidx=kkno2index(novalue);
                       if(kkparaset[kkidx].value==stvalue)
                       {
                           QMessageBox::information(this,"提示",kkparaset[kkidx].name+"已达到状态，请检查！");
                           return;
                       }
                      // kkparaset[kkidx].value=gstate;//显示
                       Man_Rosmove(kkparaset[kkidx].id);//send to mainwindow
                    }
...
void MainWindow::S_reciveb(QString ClientInfo,QByteArray data)
{
    //QMessageBox::information(this,"提示","进入Byte接收数据！");
    if(!brecvfile) return;
...
void MainWindow::add_client(QString ClientInfo)
{
    Clientcur=ClientInfo;
}

void MainWindow::remove_client(QString ClientInfo)
{
    QString datetime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
    ...
    //int row = ui->clients_list->row(Item);
    //ui->clients_list->takeItem(row);
}

void MainWindow::S_error(QString error_message)
{
    QString datetime = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss.zzz");
   // ui->server_rec->append(datetime);
...
//*********************************
    ...
    res=pthread_cancel(pthbutton);
    if(res!=0) QMessageBox::information(NULL, "提示", "調用线程BUTTON停止失败！", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
    ...
    if(client_thread->isRunning())//close web thread
    {
    ...
    {
        server_thread->quit();
        if(!server_thread->wait(3000))
        {
            QMessageBox::warning(this,"警告","server_thread->terminate");
