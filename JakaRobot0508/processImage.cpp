...
QImage ProcessImage::cvMat2QImage(const cv::Mat& mat)
{
...
cv::Mat ProcessImage::QImage2cvMat(QImage image)
{
    cv::Mat mat;
    switch(image.format())
    {
...
QImage ProcessImage::detectAndDisplay(cv::Mat frame)
{
...
ProcessImage::ProcessImage(QWidget *parent):QWidget(parent)
{
    ...
    if(!face_cascade.load(face_cascade_name)){
        recognitionFlag = 0;
        std::cout << "recognition init failed\n" << std::endl;
    }
...
void ProcessImage::AddDestIPAction(unsigned int ip,unsigned short port)
{
...
void ProcessImage::udpBroadCastInit(){
...
void ProcessImage::DeleteDestIpAction(unsigned int ip,unsigned short port)
{
    struct in_addr inaddr;
    int index;
    inaddr.s_addr = htonl(ip);
...
void ProcessImage::paintEvent(QPaintEvent *)
{
    ...
    **********/
    ...
    label->setPixmap(QPixmap::fromImage(ca->qimg));
    if(ipnum > 0){
