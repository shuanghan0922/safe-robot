...
void myGauge2::initVariables(QString ql,QString qm,QString qr)
{
    m_strleft=ql;
    m_strmind=qm;
    m_strright=qr;
      ...
      if(i>=30&&i<=70)
      {
        if(i%10==0)
        {
           // strValue=tr("%1").arg(value);
            switch(value)
            {
              case 0:strValue=m_strleft;break;
              case 20: strValue=m_strmind;break;
              case 40: strValue=m_strright;break;
            }
...
void myGauge2::drawColorPie(QPainter *painter)
{
  ...
  //  painter->setPen(Qt::NoPen);
   ...
   // painter->drawArc(m_colorCircleRect,45*16,90*16); //drawPie

    painter->drawArc(m_colorCircleRect,42*16,42*16);
    painter->drawArc(m_colorCircleRect,93*16,42*16);

  //  painter->setBrush(QColor(218,218,0));
   // painter->drawArc(m_colorCircleRect,0*16,45*16);//drawPie(m_colorCircleRect,0*16,45*16);

  //  painter->setBrush(QColor(240,50,50));
   // painter->drawArc(m_colorCircleRect,0,-45*16);//drawPie(m_colorCircleRect,0,-45*16);
