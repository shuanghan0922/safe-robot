     ...
     ***************************/
     int camerafix()
     {
         ...
         *****************/
         ...
         tem_filter.set_option(RS2_OPTION_FILTER_SMOOTH_DELTA, 20);
            ...
            //设置跳equparaset[equidx]. calibration_dz出条件20次循环或differ_flag达到条件
            int j;
            for( j=0;j<10&&differ_flag;j++)
            {
            //拍照
            ...
            //读取src
            src = imread("rs-save-to-disk-output-Color.png");
            if (!src.data)
            {
            ...
            Rect rect(cvRound(circles[i][0]) - 100, cvRound(circles[i][1]) - 100, 330, 310);
        ...
        Delay_MSec(1000);
                ...
                for(int h=0;h<10;h++)
                {
                    ...
                    filtered = spat_filter.process(filtered);
                    filtered = tem_filter.process(filtered);
                    dist_to_center = filtered.get_distance(cvRound(circles1[0][0]) + cvRound(circles[i][0]) - 100, cvRound(circles1[0][1]) + cvRound(circles[i][1]) - 100);
                    g=g+dist_to_center;
                    Delay_MSec(10);
                    dist_to_center1 = filtered.get_distance(cvRound(circles1[1][0]) + cvRound(circles[i][0]) - 100,cvRound(circles1[1][1]) + cvRound(circles[i][1]) - 100);
                    t=t+dist_to_center1;
                    Delay_MSec(10);
                    dist_to_center2 = filtered.get_distance(cvRound(circles1[2][0]) + cvRound(circles[i][0]) - 100, cvRound(circles1[2][1]) + cvRound(circles[i][1]) - 100);
                    p=p+dist_to_center2;
                    Delay_MSec(10);
                }
                dist_to_center = g/10;
                std::cout << "The camera i1s facing an object " << dist_to_center << " meters away \r"<<std::endl;
                dist_to_center1 = t/10;
                std::cout << "The camera i1s facing an object " << dist_to_center1 << " meters away \r"<<std::endl;
                dist_to_center2 = p/10;
                std::cout << "The camera i1s facing an object " << dist_to_center2 << " meters away \r"<<std::endl;
               ...
               //旋转角度计算与机械臂执行
               double  u1=sum1 + cvRound(circles1[i][0]) - 100, v1=sum2 + cvRound(circles1[i][1]) - 100,
                       u2=sum5 + cvRound(circles1[i][0]) - 100, v2=sum6 + cvRound(circles1[i][1]) - 100,
                       u3=sum9 + cvRound(circles1[i][0]) - 100, v3=sum10 + cvRound(circles1[i][1]) - 100;
               ...
               {
                   JointValue joint_pos6 = { 0 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 , 0 , angle6/2 };
                   JKrobot.joint_move(&joint_pos6, INCR, TRUE, 0.1);
                   Delay_MSec(1000);
               }
               if((angle5/2>0.00087)||(angle5/2<-0.00087))
               {
                   JointValue joint_pos5 = { 0 * PI / 180, 0 * PI / 180, 0 * PI / 180, 0 , -angle5/2 , 0 };
               ...
               //opencv
              src1 = imread("rs-save-to-disk-output-Color.png");
               ...
               Rect rect1(cvRound(circles[i][0]) - 100, cvRound(circles[i][1]) - 100, 330, 310);
               ...
               Mat img1 = src1(rect1);
                   ...
                   for(int h=0;h<10;h++)
                   {
                       ...
                       filtered = spat_filter.process(filtered);
                       filtered = tem_filter.process(filtered);
                       dist_to_center3 = filtered.get_distance(cvRound(circles2[0][0])+cvRound(circles[i][0]) - 100, cvRound(circles2[0][1]) + cvRound(circles[i][1]) - 100);
                       g1=g1+dist_to_center3;
                       Delay_MSec(10);
                       dist_to_center4 = filtered.get_distance(cvRound(circles2[1][0])+cvRound(circles[i][0]) - 100, cvRound(circles2[1][1]) + cvRound(circles[i][1]) - 100);
                       t1=t1+dist_to_center4;
                       Delay_MSec(10);
                       dist_to_center5 = filtered.get_distance(cvRound(circles2[2][0])+cvRound(circles[i][0]) - 100, cvRound(circles2[2][1]) + cvRound(circles[i][1]) - 100);
                       p1=p1+dist_to_center5;
                       Delay_MSec(10);
                   }
                   dist_to_center3 = g1/10;
                   std::cout << "The camera i1s facing an object " << dist_to_center3 << " meters away \r"<<std::endl;
                   dist_to_center4 = t1/10;
                   std::cout << "The camera i1s facing an object " << dist_to_center4 << " meters away \r"<<std::endl;
                   dist_to_center5 = p1/10;
                   std::cout << "The camera i1s facing an object " << dist_to_center5 << " meters away \r"<<std::endl;
                             ...
                             n(0,0)= sum13 + cvRound(circles[i][0]) - 100 ;
                             n(1,0)= sum23 + cvRound(circles[i][1]) - 100 ;
                             n(2,0)= 1.0;
                             ...
                             cart.tran.x = tcp_pos.tran.x + 480 - sum43 ; cart.tran.y = tcp_pos.tran.y + k1 ; cart.tran.z = tcp_pos.tran.z - k2 ;
     ...
     int move2calibrationpos(int equidx)
     {
          ...
          *******************/
          ...
          cart.tran.x = -360.502; cart.tran.y = 0; cart.tran.z = equparaset[equidx]. calibration_dz-250;
          cart.rpy.rx = 1.069 * PI / 180; cart.rpy.ry = 89.612 * PI / 180; cart.rpy.rz = -178.931 * PI / 180;
          JKrobot.linear_move(&cart, MoveMode::ABS, TRUE, sysparaset.linearspeed);
                 ...
                 while(1)//循环
            {
                 ...
                 Mat img1 = src1(rect1);
                     ...
                     for(int i=0;i<20;i++)
                     {
                         ...
                         filtered = spat_filter.process(filtered);
                         filtered = tem_filter.process(filtered);
                         dist_to_center1 = filtered.get_distance( cvRound(circles1[0][0]) + kkparaset[kkidx].pix_x, cvRound(circles1[0][1]) + kkparaset[kkidx].pix_y );
                         g1=g1+dist_to_center1;
                         Delay_MSec(100);
                     }
                     dist_to_center1 = g1/20;
                     ...
                     //opencv
                     src2 = imread("rs-save-to-disk-output-Color.png");
         ...
         for (auto i = 0; i < 30; ++i) pipe.wait_for_frames();
             ...
             Rect rect(kkparaset[kkidx].pix_x, kkparaset[kkidx].pix_y, kkparaset[kkidx].pix_w, kkparaset[kkidx].pix_h);
                 ...
                 for(int i=0;i<20;i++)
                 {
                     ...
                     filtered = spat_filter.process(filtered);
                     filtered = tem_filter.process(filtered);
                     dist_to_center = filtered.get_distance( kkparaset[kkidx].pix_x + CRect.x + CRect.width/2  ,  kkparaset[kkidx].pix_y  + CRect.y + 0.85 * CRect.height );
                     g=g+dist_to_center;
                     Delay_MSec(10);
                 }
                 dist_to_center = g/20;
                 std::cout << "The camera i1s facing an object " << dist_to_center << " meters away \r"<<std::endl;
                 qDebug("x = %d,y = %f,d = %f\n",  kkparaset[kkidx].pix_x + CRect.x + CRect.width/2 , kkparaset[kkidx].pix_y + CRect.y + 0.85 * CRect.height , dist_to_center) ;

                 n(0,0)= kkparaset[kkidx].pix_x + (CRect.x + CRect.x + CRect.width )/2  ;
                 n(1,0)= kkparaset[kkidx].pix_y + CRect.y + 0.85 * CRect.height  ;
                 n(2,0)= 1.0;
             ...
             for (auto i = 0; i < 30; ++i) pipe.wait_for_frames();
                  ...
                  for (int i = 0; i < contours31.size(); i++)
                  {
                      ...
                      for(int i=0;i<20;i++)
                      {
                          ...
                          filtered = spat_filter.process(filtered);
                          filtered = tem_filter.process(filtered);
                          dist_to_center1 = filtered.get_distance( kkparaset[kkidx].pix_x + CRect1.x + CRect1.width/2  ,  kkparaset[kkidx].pix_y  + CRect1.y + 0.85 * CRect1.height );
                          g1=g1+dist_to_center1;
                          Delay_MSec(100);
                      }
                      dist_to_center1 = g1/20;
                  ...
                  if(s>s1)
                  {
                    ...
                    if(state==0)//下拨
                    {
                        ...
                        for(int i=0;i<20;i++)
                        {
                            ...
                            filtered = spat_filter.process(filtered);
                            filtered = tem_filter.process(filtered);
                            dist_to_center = filtered.get_distance( kkparaset[kkidx].pix_x + CRect.x + CRect.width/2  ,  kkparaset[kkidx].pix_y  + CRect.y + 0.15 * CRect.height );
                            g=g+dist_to_center;
                            Delay_MSec(10);
                        }
                        dist_to_center = g/20;
                        std::cout << "The camera i1s facing an object " << dist_to_center << " meters away \r"<<std::endl;
                        qDebug("x = %d,y = %f,d = %f\n",  kkparaset[kkidx].pix_x + CRect.x + CRect.width/2 , kkparaset[kkidx].pix_y + CRect.y + 0.15 * CRect.height , dist_to_center) ;

                        n(0,0)= kkparaset[kkidx].pix_x + (CRect.x + CRect.x + CRect.width )/2  ;
                        n(1,0)= kkparaset[kkidx].pix_y + CRect.y + 0.15 * CRect.height   ;
                        n(2,0)= 1.0;
                    ...
                    for (auto i = 0; i < 30; ++i) pipe.wait_for_frames();
                         ...
                         for (int i = 0; i < contours31.size(); i++)
                         {
                             ...
                             for(int i=0;i<20;i++)
                             {
                                 ...
                                 filtered = spat_filter.process(filtered);
                                 filtered = tem_filter.process(filtered);
                                 dist_to_center1 = filtered.get_distance( kkparaset[kkidx].pix_x + CRect1.x + CRect1.width/2  ,  kkparaset[kkidx].pix_y  + CRect1.y + 0.15 * CRect1.height );
                                 g1=g1+dist_to_center1;
                                 Delay_MSec(100);
                             }
                             dist_to_center1 = g1/20;
                             ...
                             qDebug("x = %g,y = %g ,z = %g\n",  tcp_pos1.tran.x - dist_to_center1*1000 + sysparaset.dx + kkparaset[kkidx].locate_dx , tcp_pos1.tran.y + sysparaset.dy + kkparaset[kkidx].locate_dy, tcp_pos1.tran.z + sysparaset.dz + kkparaset[kkidx].locate_dz-5);

                             Delay_MSec(1000);
                             cart.tran.x = 0 ; cart.tran.y = sysparaset.dy + kkparaset[kkidx].locate_dy; cart.tran.z = sysparaset.dz + kkparaset[kkidx].locate_dz-5;
                             cart.rpy.rx = 0 ; cart.rpy.ry = 0  ; cart.rpy.rz = 0 ;
                             JKrobot.linear_move(&cart, MoveMode::INCR, TRUE, sysparaset.linearspeed);
                 ...
                 tem_filter.set_option(RS2_OPTION_FILTER_SMOOTH_DELTA, 20);
                  ...
                  for (int i = 0; i < contours31.size(); i++)
                  {
                      ...
                      for(int i=0;i<20;i++)
                      {
                          ...
                          filtered = spat_filter.process(filtered);
                          filtered = tem_filter.process(filtered);
                          dist_to_center1 = filtered.get_distance( kkparaset[kkidx].pix_x + x1 ,  kkparaset[kkidx].pix_y  + y1 );
                          g1=g1+dist_to_center1;
                          Delay_MSec(100);
                      }
                      dist_to_center1 = g1/20;
